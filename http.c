...
static void http_add_put_headers(ohc_request_t *r, char *base, ssize_t len)
{
	...
	if(r->put_header_nr == 0) {
		...
		if(p->base + p->len == base) {
			...
			if(r->put_header_nr == OHC_PUT_HEADERS_MAX) {
				return;
			}
			p++;
			p->base = base;
...
{
#define OHC_HTTP_PAGE(code, desc) \
        ...
        static struct http_page_s {
                ...
                OHC_HTTP_PAGE(201, "Created"),
                OHC_HTTP_PAGE(204, "No Content"),
                OHC_HTTP_PAGE(400, "Bad Request"),
                OHC_HTTP_PAGE(404, "Not Found"),
                OHC_HTTP_PAGE(413, "Request Entity Too Large"),
                OHC_HTTP_PAGE(416, "Requested Range Not Satisfiable"),
                OHC_HTTP_PAGE(500, "Internal Server Error"),
        };
	int i;
...
ssize_t http_decode_uri(const char *uri, ssize_t len, char *output)
{
	...
	for(i = 0; i < len; i++) {
		...
		if(uri[i] == '/' && output_len > 0 && output[output_len - 1] == '/') {
			continue;
		}
