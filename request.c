...
interupted:
	...
	r->output_size += rc;
	r->process_size += rc;
	return (length == r->process_size) ? OHC_OK : OHC_AGAIN;
}
...
static int request_write_disk(ohc_request_t *r, char *buffer, off_t length)
{
    ...
    if (REQ_PROTOCOL_MC)
    {
        size_t len = r->content_length + r->put_header_length;
        int ret = OHC_OK;

        if (r->process_size + length == len)
        {
            if (buffer[length - 1] != '\n')
            {
                ret = OHC_ERROR;
            }
            else if (length > 1 && buffer[length - 2] != '\r')
            {
                ret = OHC_ERROR;
            }
        }
        else if (r->process_size + length == (len - 1))
        {
...
static void request_do_finalize(ohc_request_t *r)
{
	...
	if(r->active) {
		...
		fprintf(fp, "%s %s %d %ld %s%s %s %s %s",
			...
			r->http_code,
			timer_now_ms(&master_timer) - r->start_time,
...
{
    if(!r->connection_broken) {
        /* don't check @request_send_buffer's return, for simple.*/
        string_t *page = NULL;

        if (REQ_PROTOCOL_MC)
        {
            page = mc_code_page(r->http_code, r->method);
            request_send_buffer(r, page->base, page->len);
        }
        else if (r->output_size == 0)
        {
            page = http_code_page(r->http_code);
            request_send_buffer(r, page->base, page->len);
        }
    }
...
static void request_put_read_request_body_preread(ohc_request_t *r)
{
    ...
    if (REQ_PROTOCOL_MC)
    {
        ssize_t max_len = REQ_BUF_SIZE;
        len = mc_make_200_response_header(max_len, buffer, r);
        r->put_header_length = len;

        if (r->item != NULL)
        {
            r->item->length = r->content_length + r->put_header_length;
            r->item->headers_len = r->put_header_length;
        }
    }
    else
    {
...
interupted:
	...
	if(!r->active) {
		r->active = 1;
		r->start_time = timer_now_ms(&master_timer);
	}

    /* mc parse */
    if (REQ_PROTOCOL_MC)
    {
        rc = mc_request_parse(r);//add by gl, change here
    }
    else
    {
    ...
    /* fprintf(r->server->access_filp, "access2 : %s %s %s\r\n", */
    /*     timer_format_log(&master_timer), */
    /*     http_methods[r->method].str.base, */
    /*     strshow(&r->uri)); */
