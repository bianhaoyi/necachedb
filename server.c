...
int server_reopen_log(unsigned short port)
{
    ohc_server_t *s = server_by_port(port);
    if(s == NULL) {
        log_error_admin(0, "no matched server");
...
inline int server_item_valid(ohc_item_t *item)
{
	return !device_of_item(item)->deleted
		...
		&& ((item->expire == 0) || (item->expire > timer_now(&master_timer)));
}
...
static ohc_hash_node_t *server_hash_get(ohc_request_t *r, unsigned char *hash_id)
{
    ...
    {
        length = r->uri.len;
        memcpy(key, r->uri.base, r->uri.len);
    }
    //http protocol
    else
    {
        ...
        if(s->key_include_host && r->host.base) {
            memlowcpy(key + length, r->host.base, r->host.len);
            length += r->host.len;
        }
    }
...
int server_request_put_handler(ohc_request_t *r)
{
    ...
    if (r->method == OHC_HTTP_METHOD_PUT)
    {
        s->sets++;
        s->sets_current_period++;
    }
    else
    {
        s->adds++;
        s->adds_current_period++;
    }
...
void server_routine(void)
{
	...
	list_for_each(p, &servers) {
		...
		if(now - s->last_clear >= s->status_period) {
			...
			s->sets_last_period = s->sets_current_period;
            s->adds_last_period = s->adds_current_period;
			s->stores_last_period = s->stores_current_period;
			s->passby_stores_last_period = s->passby_stores_current_period;
			s->sets_current_period = 0;
            s->adds_current_period = 0;
...
void server_status(FILE *filp)
{
	...
	list_for_each(p, &servers) {
        ...
        fprintf(filp,
            ...
            "adds_exist:%ld\n"
            "deletes:%ld\n"
            "deletes_last_period:%ld\n"
            "deletes_miss:%ld\n"
            "hits:%ld\n"
            "hits_last_period:%ld\n"
            "item_evict:%ld\n"
            "store:%ld\n"
            "store_last_period:%ld\n"
            "cmd_total:%ld\n"
            "passby_hits:%ld\n"
            "passby_hits_last_period:%ld\n"
            "passby_stores:%ld\n"
            "passby_stores_last_period:%ld\n"
            "output_size_last_period:%ld\n"
            "input_size_last_period:%ld\n",
            s->listen_port, s->capacity, s->status_period,
            s->consumed, s->content,
            s->item_nr, s->passby_item_nr,
            s->connections,
            s->gets, s->gets_last_period,
            s->gets_miss,
            s->sets, s->sets_last_period,
            s->adds, s->adds_last_period,
            s->adds_exist,
            s->deletes, s->deletes_last_period,
            s->deletes_miss,
            s->hits, s->hits_last_period,
            s->item_evict,
            s->stores, s->stores_last_period,
            s->gets + s->sets + s->adds + s->deletes,
