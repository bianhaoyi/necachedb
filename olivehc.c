...
static void olivehc_stats(FILE *filp)
{
    server_status(filp);
}
...
static void olivehc_admin_handler(int admin_fd)
{
    ...
    } else if(strncmp(buf, "reopenlog ", 10) == 0) {
        rc = server_reopen_log((unsigned short)atoi(buf + 10));
        if(rc == OHC_OK) {
            fputs("Server reopen log!\n", admin_out_filp);
        }
	} else {
		fputs("Invalid command!\n", admin_out_filp);
		fputs("Usage: status|stats|reload|quit|clear SERVER|reopenlog SERVER\n", admin_out_filp);
	}
